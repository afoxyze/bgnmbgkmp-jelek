...
def generate_exports():
    ...
    json_path = os.path.join(export_dir, "pbp_id_sppg_data.json")
    with open(json_path, 'w', encoding='utf-8') as f:
        json.dump(compact_data, f, indent=2, ensure_ascii=False)
    print(f"✅ JSON export created: {json_path}")

    # 2. Generate CSV
    csv_path = os.path.join(export_dir, "pbp_id_sppg_data.csv")
    ...
    cs_export_path = os.path.join(export_dir, "pbp_id_case_studies.json")
