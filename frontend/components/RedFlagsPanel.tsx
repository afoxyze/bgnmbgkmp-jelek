    ...
    <div className="flex flex-col">
      {sortedFlags.map((rf) => {
        ...
        return (
          <div
            key={rf.id}
            className="border-b border-[var(--border-base)] relative"
            style={{ backgroundColor: isActive ? "var(--bg-interactive)" : "transparent" }}
          >
            {/* Active indicator bar */}
            {isActive && <div className="absolute left-0 top-0 bottom-0 w-1 bg-red-500 z-10" />}

            <button
              onClick={() => onHighlightEntities(rf.entitas_terlibat)}
              className="w-full text-left px-6 py-4 transition-colors group cursor-pointer"
            ...
            {/* Source URLs — only expanded when this flag is the highlighted one. */}
            {isActive && hasSources && (
              <div className="px-6 pb-4 flex flex-col gap-1.5">
                <span className="text-[9px] font-bold uppercase tracking-[0.15em] text-[var(--text-tertiary)] mb-1">
                  Sumber
                </span>
                {rf.sumber.map((url, i) => (
