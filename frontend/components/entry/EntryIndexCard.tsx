...
const SEVERITY_LABELS: Record<EntryMeta["severity"], string> = {
  CRITICAL: "Sorotan",
  ACTIVE: "Berjalan",
  VERIFIED: "Dokumen Lengkap",
};

export function EntryIndexCard({ entry }: Props) {
  const focusIds = getEntryFocusIds(entry.slug);
  const graphHref =
    focusIds.length > 0
      ? `/graf?focus=${focusIds.join(",")}&from=${entry.slug}`
      : "/graf";
    ...
    <div className="relative">
      ...
      {focusIds.length > 0 && (
        <Link
          ...
          aria-label={`Buka graf dengan fokus ${focusIds.length} aktor dari ${entry.title}`}
        >
          Graf
